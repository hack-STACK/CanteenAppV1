import 'package:kantin/Models/transaction_addon_model.dart';
...
class TransactionDetailService {
  ...
  Future<List<TransactionDetail>> getTransactionDetails(
    ...
    // Query transaction details with nested menu and addon info
  ...
  Future<List<TransactionAddonDetail>> getTransactionAddons(
      int detailId) async {
    final response =
  ...
  Stream<List<TransactionDetail>> watchTransactionDetails(
    ...
    return _supabase
        ...
        .eq('transaction_id', transactionId)
        .map((rows) =>
            rows.map((row) => TransactionDetail.fromJson(row)).toList());
  }
}
